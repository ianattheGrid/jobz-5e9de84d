...
const createVRProfile = async (userId: string, fullName: string, email: string) => {
  try {
    // Using insert with partial data - the vr_number will be generated by a database trigger
    const { error } = await supabase
      ...
      .insert({
        ...
        full_name: fullName,
        email: email,
        location: 'Not specified', // Empty string instead of null for required field
