...
export const setupTestScenario = async () => {
  try {
    ...
    // Create VR profile with minimal required fields
    // vr_number will be auto-generated by database trigger
    const { error: vrProfileError } = await supabase.from('virtual_recruiter_profiles').insert({
      id: vrData.user?.id,
      email: 'test.vr@example.com',
      full_name: 'Test VR',
      location: 'Bristol',
      vr_number: 'PENDING', // This will be overwritten by the trigger
    });
